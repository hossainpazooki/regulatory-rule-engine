...
State as of 2026-07-15. Live CI: see the badges on the [README](../README.md).
...
| **6 — Revocation runtime-decision (reconciled)** | Pure `revocation_decision` (reason-class → `HardStop`/`FinishPinned`/`AuditOnly`) surfaced at `/resolve`+`/verify`; spec's platform Temporal cutover is out of scope post-ADR-0017 | **Planned — not started**; its ADR is slotted as **0024** (renumbered — 0021 was consumed by IntentSpec) | [brief](../dev/briefs/gate-6-plan-and-next-session-seed.md) |
